...
use nros_distributed::{
    FleetCoordinator, LeaderElection, NodeCapabilities, NodeInfo, NodeRole, RobotId,
    TaskRequirements,
};
...
fn main() {
    ...
    println!(
        ...
        coordinator1.leader_election.check_leader_timeout()
    );

    // Simulate heartbeat reception
    if let Some(leader) = status.leader {
        println!(
            "Current leader is {}, simulating heartbeat reception...",
            leader.0
        );
        coordinator1
            .leader_election
            .receive_heartbeat(leader, status.term);
    }

    // Distributed state management — #[shared_state(consensus="raft")] per DESIGN.md §17.1
    println!("\n=== Distributed State (Raft + ConsistentHashRing) ===");
    let ver1 = coordinator1
        .set_fleet_parameter("max_speed".to_string(), "2.0".to_string())
        .unwrap();
    let ver2 = coordinator1
        .set_fleet_parameter("formation".to_string(), "line".to_string())
        .unwrap();
    let _ver3 = coordinator1
        .set_fleet_parameter("safety_distance".to_string(), "0.5".to_string())
        .unwrap();

    println!("Set 3 params versions v{}, v{}", ver1, ver2);
    println!(
        "Fleet size: {} keys: {:?}",
        coordinator1.distributed_state.len(),
        coordinator1.distributed_state.keys()
    );

    if let Some(max_speed) = coordinator1.get_fleet_parameter("max_speed") {
        println!(
            "Fleet parameter max_speed: {} (would be replicated to 3 nodes via hash_ring)",
            max_speed
        );
    }

    // Consistent hash demo — DistributedMap per §17.1
    let shard = coordinator1
        .distributed_state
        .consistent_hash_shard("max_speed", 3);
    println!(
        "Key 'max_speed' hashes to shard {} / 3 (consistent hashing for DistributedMap)",
        shard
    );
    ...
    let pending_or_assigned: Vec<_> = {
        ...
        tasks
            .values()
            .filter(|t| {
                matches!(t.status, nros_distributed::TaskStatus::Assigned)
                    && t.assigned_to == Some(RobotId::new(1))
            })
            .cloned()
            .collect()
    };
    ...
    for task in pending_or_assigned {
        println!(
            "Robot 1 attempting to execute {} assigned to {:?}",
            task.id.0, task.assigned_to
        );
        match coordinator1.task_scheduler.execute_task(task.id) {
            Ok(dur) => println!(
                "  Completed in {:?} (GPU accelerated if available per §16.3)",
                dur
            ),
    ...
    // Also show that tasks assigned to other robots would be executed remotely
    {
        ...
        for task in tasks.values().filter(|t| {
            t.status == nros_distributed::TaskStatus::Assigned
                && t.assigned_to != Some(RobotId::new(1))
        }) {
            println!(
                "Task {} assigned to Node {:?} would execute remotely via network bridge",
                task.id.0,
                task.assigned_to.unwrap().0
            );
        }
    }
    ...
    let election2 = LeaderElection::new(RobotId::new(2))
        .with_timeouts(Duration::from_millis(200), Duration::from_millis(100));
    ...
    println!("\n=== Summary ===");
    println!(
        "✓ Leader election: term {}, role {:?}, heartbeat preventing split-brain",
        final_status.term,
        coordinator1.leader_election.role()
    );
    println!(
        "✓ Distributed state: {} keys replicated with versioning",
        coordinator1.distributed_state.len()
    );
    println!(
        "✓ Task distribution: {} tasks total, {} assigned via capability matching",
        final_status.tasks.total, final_status.tasks.assigned
    );
    println!(
        "✓ Fleet coordination: {} robots active, leader {:?}",
        final_status.total_robots, final_status.leader
    );
