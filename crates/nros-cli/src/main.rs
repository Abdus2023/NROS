...
use nros_cli::{BuildProfile, Command, TopicAction, CLI};
use std::env;
    ...
    let result = match cmd {
        "init" => {
            ...
            let template = args
                ...
                .map(|s| s.split('=').nth(1).unwrap_or("basic").to_string());
        ...
        "build" => {
            let profile_str = args
                ...
                .map(|s| s.split('=').nth(1).unwrap_or("debug"))
                .unwrap_or("debug");
        ...
        "run" => {
            ...
            let node = if args.len() > 2 && !args[2].starts_with("--") {
                Some(args[2].clone())
            } else {
                None
            };
        ...
        "topic" => {
            ...
            let action = match args[2].as_str() {
                ...
                "info" => TopicAction::Info {
                    name: args
                        .get(3)
                        .cloned()
                        .unwrap_or_else(|| "/cmd_vel".to_string()),
                },
                "echo" => TopicAction::Echo {
                    name: args
                        .get(3)
                        .cloned()
                ...
                "bw" => TopicAction::Bw {
                    name: args
                        .get(3)
                        .cloned()
                        .unwrap_or_else(|| "/camera/image".to_string()),
                },
                "pub" => TopicAction::Pub {
                    name: args
                        .get(3)
                        .cloned()
                        .unwrap_or_else(|| "/cmd_vel".to_string()),
                    data: args.get(4).cloned().unwrap_or_else(|| "{}".to_string()),
                },
                _ => TopicAction::List,
            };
        ...
        "record" => {
            ...
            let output = args
                ...
                .map(|s| PathBuf::from(s.split('=').nth(1).unwrap_or("recording.nros")))
                .unwrap_or_else(|| PathBuf::from("recording.nros"));
            ...
            let res = CLI::run(Command::Record {
                topics,
                output,
                duration,
            });
        ...
        "replay" => {
            ...
            let res = CLI::run(Command::Replay {
                input,
                speed,
                loop_playback,
            });
        ...
        "analyze" => {
            ...
            let res = CLI::run(Command::Analyze {
                input,
                analysis_type,
            });
        ...
        "fleet" => {
            ...
            let action = match args[2].as_str() {
                ...
                "deploy" => {
                    let version = args
                        .iter()
                        .find(|a| a.starts_with("--version="))
                        .map(|s| s.split('=').nth(1).unwrap_or("1.0.0").to_string())
                        .unwrap_or_else(|| "1.0.0".to_string());
                    let canary = args.iter().find(|a| a.starts_with("--canary=")).map(|s| {
                        s.split('=')
                            .nth(1)
                            .unwrap_or("0")
                            .trim_end_matches('%')
                ...
                "exec" => {
                    let robot = args
                        .get(3)
                        .cloned()
                        .unwrap_or_else(|| "robot_001".to_string());
                    let command = args
                        .get(4)
                        .cloned()
            ...
            let action = match args[2].as_str() {
                ...
                "convert" => {
                    ...
                    let output = args
                        .get(4)
                        .map(|s| PathBuf::from(s))
                        .unwrap_or_else(|| PathBuf::from("nros_pkg"));
