...
impl ProjectInitializer {
    pub fn init(name: &str, template: Option<&str>) -> Result<PathBuf, String> {
        ...
        // Create nros.toml per §20.1 (NROS project metadata)
        ...
        // Pass 24 (CI-001): also create a real Cargo.toml so the generated project
        // is actually `cargo check`-able. Previously the generator wrote only
        // `nros.toml`, so `cat Cargo.toml` in CI and `cargo build` by a user both failed.
        let cargo_toml = Self::generate_cargo_toml(name);
        ...
        // Create sample node — write to BOTH src/main.rs (the cargo binary entry point,
        // needed so `cargo check` works) and src/nodes/main.rs (the DESIGN-documented path).
        let node_content = Self::generate_sample_node(template);
        let main_path = base.join("src/main.rs");
        std::fs::write(&main_path, &node_content)
...
"#,
    ...
    /// Generate a real Cargo.toml for the generated project (Pass 24, CI-001).
    /// The generated template is standalone Rust (no external NROS deps), so it
    /// compiles with just the standard library. This makes `cargo check` and the
    /// CI nros-init golden test meaningful.
...
version = "0.1.0"
edition = "2021"
description = "NROS project generated by `nros init`"

[dependencies]
# The generated templates are standalone (no NROS dependency required to build).
# To use the full NROS API, add:
# nros = {{ path = "../nros/crates/nros" }}
