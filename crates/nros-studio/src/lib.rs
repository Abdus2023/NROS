...
impl DataProvider for DemoDataProvider {
    fn get_nodes(&self) -> HashMap<String, NodeInfo> {
        ...
        nodes.insert(
            ...
            NodeInfo {
                ...
                deadline_misses: 0,
                params: [
                    ("max_speed".into(), "2.0".into()),
                    ("wheel_base".into(), "0.5".into()),
                ]
                .into_iter()
                .collect(),
            },
        ...
        nodes.insert(
            ...
            NodeInfo {
                ...
                priority: 100,
                deadline_misses: 0,
                params: [("algorithm".into(), "HybridAStar".into())]
                    .into_iter()
                    .collect(),
            },
        );
    ...
    fn get_topics(&self) -> HashMap<String, TopicInfo> {
        ...
        topics.insert(
            ...
            TopicInfo {
                name: "/cmd_vel".into(),
                type_name: "geometry_msgs/Twist".into(),
                rate_hz: 10.0,
                ...
                subscribers: vec!["motor_driver".into()],
                qos: "realtime".into(),
            },
        );
        topics
    ...
    fn name(&self) -> &'static str {
        "DemoDataProvider (SIMULATED — pseudo_rand)"
    }
}
...
impl StudioState {
    pub fn new() -> Self {
        ...
        nodes.insert(
            ...
            NodeInfo {
                ...
                params: [
                    ("max_speed".into(), "2.0".into()),
                    ("wheel_base".into(), "0.5".into()),
                    ("cmd_timeout_ms".into(), "500".into()),
                    ("safety_limits_enabled".into(), "true".into()),
                ]
                .into_iter()
                .collect(),
            },
        ...
        nodes.insert(
            ...
            NodeInfo {
                ...
                deadline_misses: 1,
                params: [
                    ("range".into(), "10.0".into()),
                    ("num_rays".into(), "360".into()),
                ]
                .into_iter()
                .collect(),
            },
        ...
        nodes.insert(
            ...
            NodeInfo {
                ...
                deadline_misses: 0,
                params: [
                    ("algorithm".into(), "HybridAStar".into()),
                    ("resolution".into(), "0.05".into()),
                ]
                .into_iter()
                .collect(),
            },
        ...
        topics.insert(
            ...
            TopicInfo {
                name: "/odom".into(),
                type_name: "nav_msgs/Odometry".into(),
                rate_hz: 50.0,
        ...
        topics.insert(
            ...
            TopicInfo {
                name: "/scan".into(),
                type_name: "sensor_msgs/LaserScan".into(),
                rate_hz: 10.0,
                ...
                publishers: vec!["lidar_processor".into()],
                subscribers: vec!["path_planner".into()],
    ...
    pub fn to_status_json(&self) -> String {
        ...
        format!(
            r#"{{"uptime_sec":{},"nodes":{{{}}},"topics":{},"alerts":{},"tf_frames":{}}}"#,
            uptime,
            nodes_json,
            self.topics.len(),
            self.alerts.len(),
    ...
    pub fn to_metric_json(&self) -> String {
        ...
        format!(
            r#"{{"timestamp_ms":{},"latency_us":{:.2},"throughput_kmsg":{:.1},"cpu_pct":{:.1},"memory_gb":{:.2},"deadline_misses":{},"uptime_sec":{},"provider":"{}","simulated":{}}}"#,
            metric.timestamp_ms,
            metric.latency_us,
            metric.throughput_kmsg,
            metric.cpu_pct,
            metric.memory_gb,
            metric.deadline_misses,
            uptime,
            self.data_provider.name(),
            self.data_provider.is_simulated()
        )
    }
    ...
    pub fn update_param(&mut self, node: &str, key: &str, value: &str) -> Result<(), String> {
        ...
        println!(
            "[Studio] Param updated: {}/{} = {} (hot-reload per §17.3 validation @range)",
            node, key, value
        );
        Ok(())
    }
...
impl StudioServer {
    pub fn new(addr: &str) -> Self {
        Self {
            addr: addr.to_string(),
            state: Arc::new(Mutex::new(StudioState::new())),
        }
    }
    ...
    pub fn run(&self) -> std::io::Result<()> {
        ...
        for p in &[
            "crates/nros-studio/static/index.html",
            "static/index.html",
            "studio/index.html",
        ] {
            if let Ok(c) = std::fs::read_to_string(p) {
                html_content = c;
                println!("   Serving from {}", p);
                break;
            }
        }
        if html_content.is_empty() {
            html_content =
                "<html><body><h1>NROS Studio</h1><p>Dashboard not found</p></body></html>"
                    .to_string();
        }
    ...
    fn handle_client(
        ...
        match (method, path_base) {
            ...
            ("POST", "/api/params") | ("GET", "/api/params") => {
                ...
                let query_str = if path.contains('?') {
                ...
                let combined_query = if !body_part.is_empty() {
                    body_part
                } else {
                    query_str
                };
                ...
                let node = params_map
                    .get("node")
                    .cloned()
                    .unwrap_or_else(|| "velocity_controller".to_string());
                let key = params_map
                    .get("key")
                    .cloned()
                    .unwrap_or_else(|| "max_speed".to_string());
            ...
            _ => Self::send_response_with_status(
                &mut stream,
...
mod tests {
    ...
    fn test_param_update() {
        ...
        assert!(state
            .update_param("velocity_controller", "max_speed", "3.0")
            .is_ok());
        assert_eq!(
            state
                .nodes
                .get("velocity_controller")
                .unwrap()
                .params
                .get("max_speed")
                .unwrap(),
