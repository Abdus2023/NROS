...
use nros_transport::{CompressionEngineTrait, Lz4CompressionEngine, MockCompressionEngine};
...
fn main() {
    ...
    println!(
        "Mock should_compress: {} (threshold 1024)",
        mock.should_compress(&data)
    );
    ...
    println!(
        ...
        data.len(),
        mock_compressed.len(),
        mock_compressed.len() as f64 / data.len() as f64,
        mock.name(),
        mock.is_simulated()
    );
    ...
    println!(
        "Lz4 compression: {} -> {} bytes, ratio {:.2}, name: {}, simulated: {}",
        data.len(),
        lz4_compressed.len(),
        lz4_compressed.len() as f64 / data.len() as f64,
        lz4.name(),
        lz4.is_simulated()
    );
    ...
    println!(
        "\nMock decompress ok: {}",
        mock_decompressed.len() == data.len()
    );
    ...
    #[cfg(feature = "real-compression")]
    {
        ...
        println!(
            "Lz4 decompress ok (real lz4_flex): {} -> {} bytes, ratio {:.2} measured, not assumed",
            data.len(),
            lz4_compressed.len(),
            lz4_compressed.len() as f64 / data.len() as f64
        );
    ...
    println!(
        "- Lz4Compression: {} — would use lz4_flex, currently {}",
        if cfg!(feature = "real-compression") {
            "REAL (lz4_flex)"
        } else {
            "SCAFFOLDED [2]+data"
        },
        if cfg!(feature = "real-compression") {
            ...
            "flag only, needs --features real-compression"
        }
    );
}
