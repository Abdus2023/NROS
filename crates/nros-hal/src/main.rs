...
use nros_hal::{
    CameraDriver, ImageFormat, ImuDriver, LidarDriver, Sensor, SensorConfig, SensorManager,
    SensorSynchronizer, TriggerMode,
};
...
fn main() {
    ...
    manager.register_sensor("imu".to_string(), Box::new(ImuDriver::new("IMU")));
    ...
    lidar
        .configure(SensorConfig {
            rate_hz: 10.0,
            trigger_mode: TriggerMode::External { pin: 4 },
            ..Default::default()
        })
        .unwrap();
    imu.configure(SensorConfig {
        rate_hz: 200.0,
        use_dma: true,
        ..Default::default()
    })
    ...
    println!(
        "Capturing 5 synchronized frames (simulating 30Hz camera, 10Hz LiDAR, 200Hz IMU)...\n"
    );
    for i in 0..5 {
        ...
        if let Ok(frame) = camera.capture_frame() {
            println!(
                "Frame {} captured: {}x{} {:?} ({} bytes) DMA buf: {:?}",
                frame.frame_id,
                frame.width,
                frame.height,
                frame.format,
        ...
        if i % 3 == 0 {
            if let Ok(scan) = lidar.capture_scan() {
                println!(
                    "Scan {} captured: {} points ({} bytes)",
                    scan.scan_id,
                    scan.points.len(),
                    scan.points.len() * std::mem::size_of::<nros_hal::Point3D>()
                );
        ...
        if let Some(synced) = synchronizer.try_synchronize() {
            println!(
                "✓ Synchronized set {} ready! sync_quality={}ms, ts={}ms",
                i + 1,
                synced.sync_quality_ms,
                synced.timestamp.to_millis()
            );
        } else {
            let (c, l, imu_c) = synchronizer.buffered_counts();
            println!(
                "  Buffers: cam={}, lidar={}, imu={} — waiting for sync within 10ms...",
                c, l, imu_c
            );
        }
        println!();
    ...
    println!(
        "Sync success rate: {:.1}% ({} / {} attempts)",
        synchronizer.success_rate(),
        synchronizer.sync_success,
        synchronizer.sync_attempts
    );
    ...
    for j in 0..3 {
        ...
        println!(
            "  DMA frame {}: GPU buffer id {:?}, ptr={:p} (would launch CUDA kernel directly)",
            j,
            frame.dma_buffer_id,
            frame.data.as_ptr()
        );
