...
pub fn run(){
 ...
 let architecture=read("architecture.yaml");let capabilities=read("capabilities.yaml");let evidence=read("evidence.yaml");let claims=read("claims.yaml");let canonical=fs::read_to_string("docs/REPOSITORY_REPRESENTATION.md").unwrap_or_default();let mut failures=0;
 ...
 let ev=records(&evidence,"- capability: ");let mut ev_caps=BTreeSet::new();
 for(cap,f)in&ev{require(ev_caps.insert(cap.clone()),&format!("evidence {} unique",cap),&mut failures);require(ids.contains(cap),&format!("evidence {} references known capability",cap),&mut failures);for k in ["source","tests","ci","miri","benchmark","hardware"]{require(f.contains_key(k)||evidence.contains(&format!("{}:\n",k)),&format!("evidence {} has {} dimension",cap,k),&mut failures)}}
 for cap in &ids{require(ev_caps.contains(cap),&format!("capability {} has evidence record",cap),&mut failures)}
 let cls=records(&claims,"- id: ");let mut claim_ids=BTreeSet::new();
 for(id,f)in&cls{require(claim_ids.insert(id.clone()),&format!("claim {} unique",id),&mut failures);require(f.contains_key("subject"),&format!("claim {} has subject",id),&mut failures);require(f.get("class").is_some_and(|c|CLAIM_CLASSES.contains(&c.as_str())),&format!("claim {} has valid class",id),&mut failures)}
 ...
 for n in ["configured_ci_is_not_passed_ci","benchmark_artifact_is_not_independent_validation","simulated_implementation_cannot_support_real_backend_claim","hardware_validation_requires_actual_hardware_evidence"]{require(evidence.contains(n),&format!("evidence invariant {}",n),&mut failures)}
 for n in ["no_claim_without_evidence_record","no_real_claim_from_simulated_backend","no_ci_pass_claim_without_executed_successful_run"]{require(claims.contains(n),&format!("claim invariant {}",n),&mut failures)}
 for n in ["source_of_truth: DESIGN.md","architecture_intent_is_not_implementation_evidence","crate_topology_is_not_runtime_topology"]{require(architecture.contains(n),&format!("architecture invariant {}",n),&mut failures)}
 require(canonical.contains("docs/representation/"),"canonical representation directory",&mut failures);require(canonical.contains("No specification implies implementation."),"canonical specification invariant",&mut failures);
