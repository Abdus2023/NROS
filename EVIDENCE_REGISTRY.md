...
|---------|------|----------------|--------|------|-----------|---------------|---------------|
| SPSC ring buffer lock-free | DESIGN.md §14.1 | `crates/nros-core/src/lib.rs` RingBuffer | IMPLEMENTED → TESTED after Safety Gate v0.1 → **BENCHMARKED (same-thread only)** | `test_spsc_ordering`, `test_double_reserve_prevention` — full `nros-core` suite green in CI (run 32707535971) and 20 passed / 1 ignored locally | `benchmark_latency_monotonic` still `#[ignore]` and still pushes a hard-coded `1000` where the publish-instant delta belongs (loose end). Real evidence: `probes/microbench.rs` 110.90 ns/op same-thread; `results_e2b-sandbox-2vcpu_20260824.json` cross-thread | Not required | Yes (with guard API). Latency claims limited to the same-thread figure — see §Performance Claims |
