...
|---------|------|----------------|--------|------|-----------|---------------|---------------|
| SPSC ring buffer lock-free | DESIGN.md §14.1 | `crates/nros-core/src/lib.rs` RingBuffer | IMPLEMENTED → TESTED after Safety Gate v0.1 → **BENCHMARKED (same-thread only)** → **Miri PASS (run 32718667809)** | `test_spsc_ordering`, `test_double_reserve_prevention` — full `nros-core` suite green; **Miri green on nros-core + nros-types (run 32718667809)**; Pass 31 closed the SPSC ownership hole (P31-01: deprecated `Publisher::ring()`/`from_ring()`/`Subscriber::new(Arc)` removed, sole path `Publisher::declare`) and wired loom models (P31-02, execution pending — CI job staged as `docs/audit/P31-02-ci-loom-job.patch`) | Real evidence: `probes/microbench.rs` 110.90 ns/op same-thread; `results_e2b-sandbox-2vcpu_20260824.json` cross-thread | Not required | Yes (with guard API). Latency claims limited to the same-thread figure — see §Performance Claims. Loom interleaving proof not yet executed |
...
| TCP transport | DESIGN.md §14.3 | `TcpListener`, `TcpStream`, `set_nodelay`; per-connection `rx_buf` reassembly | IMPLEMENTED → TESTED | `test_tcp_fragmented_delivery_no_desync` (forces half-header / byte-at-a-time fragmentation) | Pending | N/A | Yes (basic). **Pass 29 F29-01:** `receive()` returned `Err` on peer EOF *before* parsing, discarding a complete checksum-valid frame already in `rx_buf` — a clean shutdown lost the last frame sent, and the resulting race made `cargo test` nondeterministic (6/40 local runs; same CI head_sha passed in one run and failed in another). Fixed: EOF ends the drain, not the call. 0/100 after the fix |
...
|-------|--------|-------------------|----------------|---------------|
| 6.2 μs mean latency, 780K msg/s | README.md Achieved | Benchmark artifact with CPU model, OS, compiler, commit, affinity, iterations, distribution (per AUDIT Pass 7 §12) | **Pass 29: the downgrade recorded here has been applied to README.md and COMPARISON.md.** The figure traces to `benchmarks/results.json`, which declares itself a TEMPLATE carrying unverified numbers. Real runs of the harness now exist and disagree by 1–2 orders of magnitude: 156.52 μs mean / 3.34M msg/s (`results_e2b-sandbox-2vcpu_20260824.json` @ `835165df`) and 588.21 μs mean (Pass 27 artifact) — both cross-thread and scheduler-bound on shared unpinned 2-vCPU sandboxes. Same-thread SPSC, which is what characterises the ring, measures **110.90 ns/op (9.02M msg/s)**, reproducing Pass 27's 112 ns/op | 🟡 **Downgraded and applied.** No executed measurement has produced 6.2 μs; the only claim allowed is "target <10 μs, same-thread ring cost 110.90 ns/op measured, cross-thread latency scheduler-bound on 2 shared vCPU". `<10 μs` needs CPU pinning on target hardware |
| 46x faster than ROS2, 15x throughput, etc. | COMPARISON.md | Independent reproducible benchmark ROS2 vs NROS same hardware, same message size, same conditions | No ROS2 baseline in this repo. Pass 29: a scoping caveat was added above the §2.1 table and the "NROS Strengths" bullets were reworded, so the ratios are no longer presented bare | 🔴 Not independently established — **caveat now published alongside the table**; the ratios remain design targets |
